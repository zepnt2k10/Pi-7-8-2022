...
\newpage
\begingroup
\AddToShipoutPicture*{\put(170,680){\includegraphics[scale=1]{../tieude3.pdf}}} 
\centering
\endgroup
\graphicspath{{../toancuabi/pic/}}
\vspace*{25pt}
...
\begin{multicols}{2}
	...
	\vskip 0.1cm
	\PIbox{
	\textbf{\color{toancuabi}Problem} $\pmb{1}$ ({\color{toancuabi}Magic Triangle of order $3$ and magic sum $9$})\textbf{\color{toancuabi}.} Arrange the numbers from $1$ to $6$ into the circles in the triangle above so that the magic sum is equal to $9$.}
	\begin{figure}[H]
		\vspace*{-5pt}
		\centering
		\captionsetup{labelformat= empty, justification=centering}
	...
	\begin{figure}[H]
		...
		\vspace*{-10pt}
	\end{figure}
	We can either flip or rotate a given Magic Triangle and a new set of Magic Triangle will appear:
	\begin{figure}[H]
		\vspace*{-10pt}
		\centering
		\captionsetup{labelformat= empty, justification=centering}
		\begin{tikzpicture}[scale=0.75]
			...
			\begin{scriptsize}
				...
				\draw(4,-0.9) node[roundnode] {$1$};
				\draw(3.25,-2.2) node[roundnode] {$5$};
				\draw(4.75,-2.2) node[roundnode] {$6$};
				\draw(2.5,-3.5) node[roundnode] {$3$};
				\draw(4,-3.5) node[roundnode] {$4$};
				\draw(5.5,-3.5) node[roundnode] {$2$};
			\end{scriptsize}
	...
	\vskip 0.05cm
	\textbf{\color{toancuabi}Problem $\pmb{3.}$} In the picture below, there are three big circles, each of which intersects the other at four small cirles (colored white). Arrange the numbers from $1$ to $6$ into the small circles so that the sum of numbers on each big circle is equal to $14$.
	\begin{figure}[H]
		\vspace*{-5pt}
		\centering
		\captionsetup{labelformat= empty, justification=centering}
	...
	\begin{figure}[H]
		...
		\vspace*{-10pt}
	\end{figure}
	As shown on the diagram, we call the numbers in the small circles $a$, $b$, $c$, $d$, $e$, $f$, $g$, $h$. Thus, we have:
	\begin{align*}
		a+b+c+d+e+f &= 21,\tag{$1$}\\
		a+b+d+e &= 14,\tag{$2$}\\
		a+f+d+c &= 14.     \tag{$3$}
	...
	\begin{align*}
		7 = 1+6 = 2+5 = 3+4.
	\end{align*} 
	Therefore, we could fill in the two--number sets $(1,6)$, $(2,5)$, $(3,4)$ in any order into the pairs of white circles $(a,d)$, $(c,f)$, $(e,b)$, and we will get different results for each choice. Two such examples are:
	\begin{figure}[H]
		\vspace*{-5pt}
		\centering
		\captionsetup{labelformat= empty, justification=centering}
	...
	\vskip 0.1cm
	{\color{toancuabi}arrangement} (n): một cách sắp xếp
	\vskip 0.1cm	
	{\color{toancuabi}partition}: phân hoạch}
\end{multicols}